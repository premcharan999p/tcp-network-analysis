...
Modified LAN Topology Implementation for TCP Performance Analysis
"""
...
class LanTopology(Topo):
    """
    LAN topology with configurable number of hosts and switches
    """
    def build(self, n=4):
        # Add switch
        ...
        for h in range(n):
            ...
            # Add link with 100Mbps bandwidth, 2ms delay
...
def perfTest(tcp_variant):
    ...
    topo = LanTopology(n=2)  # Using only 2 hosts for simplicity
    ...
    server_cmd = f'iperf -s -i 1 > results/lan/iperf_server_lan_{tcp_variant}.log 2>&1 &'
    h2.cmd(server_cmd)
    ...
    client_cmd = f'iperf -c {h2.IP()} -t 30 -i 1 > results/lan/iperf_client_lan_{tcp_variant}.log 2>&1'
    h1.cmd(client_cmd)
    ...
    h1.cmd(f'ss -i > results/lan/ss_stats_lan_{tcp_variant}.log 2>&1')
...
if __name__ == '__main__':
    ...
    print("*** Running LAN performance test with TCP Tahoe/Reno")
    success_reno = perfTest('reno')  # Using Reno as a substitute for Tahoe as it's not directly available
    
    print("*** Running LAN performance test with TCP Cubic")
    success_cubic = perfTest('cubic')
    
    if success_reno and success_cubic:
        print("*** All LAN tests completed successfully")
    else:
        print("*** Some LAN tests failed")
